import ujson, ure, time, gc, _thread, urequests
from machine import Timer


_thread.stack_size(80960)
...
class Bot():
    ...
    def __init__(self, p_token, p_func_connectar_xarxa):
        self._func_connectar_xarxa = p_func_connectar_xarxa
        self._xarxa = self._func_connectar_xarxa()
        self.url = 'https://api.telegram.org/bot' + p_token
        self.last_update = 0
    ...
    def _get_updates(self):
        ...
        parameters = { 'offset': self.last_update + 1,
                       'timeout': 2,
                       'allowed_updates': ['messages'] }
        try:
            headers = {'Content-type': 'application/json', 'Accept': 'text/plain'}

            response = urequests.post(self.url + '/getUpdates', json=parameters, headers=headers)
...
#            print('*******************', self.last_update)
    ...
    def _handle_update(self, update):
        ...
        if text.startswith('/'): #is a command
            ...
            if command in set(self.command_handlers.keys()):
                print('verifica comanda', command)
    ...
    def _loop(self):
        while True:
            if self._xarxa.isconnected():
                self._read()
            else:
                self._xarxa = self._func_connectar_xarxa()
        
    def start_loop(self, main_function=None, args=()):
        """
    ...
    def send_message(self, chat_id, text, parse_mode='MarkdownV2', reply_markup=None):
        ...
        try:
            headers = {'Content-type': 'application/json', 'Accept': 'text/plain'}
            response = urequests.post(self.url + '/sendMessage', json=parameters, headers=headers)
            assert response
            response.close()
        except Exception:
    ...
    def update_message(self, chat_id, message_id, text, parse_mode='MarkdownV2', reply_markup=None):
        ...
        try:
            headers = {'Content-type': 'application/json', 'Accept': 'text/plain'}
            response = urequests.post(self.url + '/editMessageText', json=parameters, headers=headers)
            assert response
            response.close()
        except Exception:
...
class Update():
    ...
    def edit(self,text, parse_mode='MarkdownV2', reply_markup=None):
        self.bot.update_message(self.message['chat']['id'] , self.message['message_id'], 
